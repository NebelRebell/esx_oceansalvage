...
function SelectOceansalvage()
	local index = GetRandomIntInRange(1, #Config.Oceansalvage)

	for k,v in pairs(Config.Zones) do
		if v.Pos.x == Config.Oceansalvage[index].x and v.Pos.y == Config.Oceansalvage[index].y and v.Pos.z == Config.Oceansalvage[index].z then
			return k
		end
	end
end
...
function StartNPCJob()
	NPCTargetOceansalvage = SelectOceansalvage()
	local zone = Config.Zones[NPCTargetOceansalvage]

	Blips['NPCTargetOceansalvage'] = AddBlipForCoord(zone.Pos.x, zone.Pos.y, zone.Pos.z)
	SetBlipRoute(Blips['NPCTargetOceansalvage'], true)
	ESX.ShowNotification(_U('GPS_info'))
	Done = true
end

function StopNPCJob(cancel)
	if Blips['NPCTargetOceansalvage'] ~= nil then
		RemoveBlip(Blips['NPCTargetOceansalvage'])
		Blips['NPCTargetOceansalvage'] = nil
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if NPCTargetOceansalvage ~= nil then
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(1, Keys["DELETE"]) and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == Config.JobName then
